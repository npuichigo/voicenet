...
class AcousticModel(snt.AbstractModule):
    """A deep RNN model, for use of acoustic modeling."""
    ...
    def __init__(self, rnn_cell, num_hidden, dnn_depth, rnn_depth, output_size,
        ...
        super(AcousticModel, self).__init__(name=name)

        if rnn_cell == 'rnn':
            self._cell_fn = tf.contrib.rnn.BasicRNNCell
        elif rnn_cell == 'gru':
            self._cell_fn = tf.contrib.rnn.GRUCell
        elif rnn_cell == 'lstm':
            self._cell_fn = tf.contrib.rnn.BasicLSTMCell
        else:
        ...
        with self._enter_variable_scope():
            ...
            if not self._bidirectional:
                ...
                self._core = tf.contrib.rnn.MultiRNNCell(self._rnns)
            else:
                self._core = {
                    "fw": tf.contrib.rnn.MultiRNNCell([
                        self._cell_fn(self._num_hidden)
                        for i in range(self._rnn_depth)
                    ]),
                    "bw": tf.contrib.rnn.MultiRNNCell([
                        self._cell_fn(self._num_hidden)
                        for i in range(self._rnn_depth)
                    ]),
                }
    ...
    def _build(self, input_sequence, input_length):
        ...
        batch_input_sequence = batch_input_module(input_sequence)

        if not self._bidirectional:
            output_sequence, final_state = tf.nn.dynamic_rnn(
                cell=self._core,
                inputs=batch_input_sequence,
