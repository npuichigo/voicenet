...
extern "C" {
  ...
  void reaper(float_list *input, int length, double samplerate,
...
void reaper(float_list *input, int length, double samplerate, int frame_shift,
  ...
  float external_frame_interval = static_cast<float>(frame_shift / samplerate);
  ...
  for (int i = 0; i < f0->num_frames(); ++i) {
    for (int j = 0; j < f0->num_channels(); ++j) {
      float f;
      switch (otype) {
        case 1: {
          if (f0->v(i)) {
